...
The accompanying reproducibility repository is publicly available at \url{https://github.com/ksumuluz/differentiable-ultrametric-routing}. It records the exact ModernBERT model revision, embedding-extraction configuration, split definition, label-consistency audit, validation-selected routing parameters, all ten seed-level outputs, matched MAP-plus-finite-bias ablation, paired seed statistics, document-bootstrap outputs, theorem-verification results and executable verification code, and figure/table regeneration scripts. Candidate hyperparameter grids are specified in the executable WOS pipeline, while selected hyperparameters and full per-seed outputs remain machine-readable rather than duplicated in the manuscript.
